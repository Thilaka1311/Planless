-- Fix create_wallet_settlement to use PENDING and SETTLED
CREATE OR REPLACE FUNCTION public.create_wallet_settlement(p_other_user_id uuid, p_amount numeric, p_plan_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
-- Fix delete_wallet_settlement to use PENDING and SETTLED
CREATE OR REPLACE FUNCTION public.delete_wallet_settlement(p_settlement_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
-- Fix recalculate_wallet_expenses to use PENDING and SETTLED
...
BEGIN
  ...
  -- Upsert participant shares for all JOINED and WAITLISTED participants, PRESERVING existing 'SETTLED' status and amount_paid
