-- Migration: Fix complete_plan expense calculation and loop variables
-- Description: Ensures v_input_attendance is reset per participant, and v_share correctly falls back to non-zero values in KEEP_CURRENT_COST mode. Also ensures amount_owed is reliably upserted.
  ...
  IF p_expense_mode IN ('SPLIT_ALL', 'KEEP_CURRENT_COST') AND v_final_count > 0 THEN
    ...
    IF v_plan_expense IS NOT NULL THEN
      ...
      ELSIF p_expense_mode = 'KEEP_CURRENT_COST' THEN
        ...
        WHERE expense_id = v_plan_expense.id AND status != 'SKIPPED' AND amount_owed > 0
