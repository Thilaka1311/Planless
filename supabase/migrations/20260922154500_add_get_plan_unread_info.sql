-- Migration: 20260922154500_add_get_plan_unread_info.sql
-- Function to retrieve the exact unread count and the first unread message ID for a user in a plan
...
AS $$
  ...
  SELECT
    ...
    ur.last_read_at,
    COALESCE((SELECT COUNT(*) FROM unreads), 0) AS unread_count,
