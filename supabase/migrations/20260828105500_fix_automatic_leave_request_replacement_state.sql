-- Migration: 20260828105500_fix_automatic_leave_request_replacement_state.sql
-- Description: Update resolve_paid_plan_leave_request and replace_participant RPCs so that when a participant is replaced, the leaving participant's rsvp_status transitions to INVITED (instead of SKIPPED) and the replacement participant transitions to JOINED (instead of INVITED).
...
BEGIN
  ...
  IF p_resolution = 'REPLACED' THEN
    ...
    -- Update target participant to INVITED / REPLACED
  ...
  IF p_replacement_user_id IS NOT NULL THEN
    ...
    IF NOT FOUND THEN
      -- Insert replacement user directly as JOINED/GOING
      INSERT INTO public.plan_participants (
        plan_id, user_id, rsvp_status, role, assigned_group, responded_at
      ) VALUES (
        p_plan_id, p_replacement_user_id, 'JOINED'::rsvp_status, 'PARTICIPANT'::participant_role,
        CASE WHEN v_filtering_mode = 'ASSIGNED'::participant_filtering_type THEN 'GOING'::assigned_group_enum ELSE NULL END, now()
      );
    ELSE
      IF v_filtering_mode = 'ASSIGNED'::participant_filtering_type AND v_replacement_assigned_group = 'WAITLIST'::assigned_group_enum THEN
        -- Assigned Waitlist Replacement Logic: promote to GOING and JOINED
        UPDATE public.plan_participants
           SET assigned_group = 'GOING'::assigned_group_enum,
               rsvp_status = 'JOINED'::rsvp_status,
               waitlist_position = NULL,
        ...
        UPDATE public.plan_participants pp
         ...
         WHERE pp.plan_id = p_plan_id AND pp.user_id = n.user_id;
      ELSE
        -- Standard replacement logic (Automatic waitlist or external participant): promote to JOINED
        UPDATE public.plan_participants
...
BEGIN
  ...
  -- 5. Transition target participant to INVITED / REPLACED
  UPDATE public.plan_participants
     SET rsvp_status        = 'INVITED'::rsvp_status,
         skip_reason        = 'REPLACED'::skip_reason,
         assigned_group     = NULL,
  ...
  -- 6. Transition or Insert replacement participant to JOINED / GOING
  IF FOUND THEN
