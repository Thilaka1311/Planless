-- Fix: Remove invalid 'SKIPPED' enum usage in complete_plan expense block
...
BEGIN
  ...
  FOR v_participant IN
    ...
    FOR UPDATE
  LOOP
    -- CRITICAL FIX: Reset the loop variable for each participant!
  ...
  IF p_expense_mode IN ('SPLIT_ALL', 'KEEP_CURRENT_COST') AND v_final_count > 0 THEN
    ...
    -- FIX: Use v_plan_expense.id IS NOT NULL instead of v_plan_expense IS NOT NULL
    -- because in Postgres, a RECORD is neither NULL nor NOT NULL if it contains a mix of null and non-null fields!
    IF v_plan_expense.id IS NOT NULL THEN
      ...
      ELSIF p_expense_mode = 'KEEP_CURRENT_COST' THEN
        -- CRITICAL FIX: Ensure we pick a valid, non-zero amount if possible.
        -- Do not use 'SKIPPED' as a status check since participant_payment_status only contains PENDING/SETTLED.
      ...
      LOOP
        ...
        ELSE
          ...
          -- (Preserving existing SETTLED records per lifecycle rules)
