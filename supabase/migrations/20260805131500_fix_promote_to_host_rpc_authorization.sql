-- Migration: Update promote_to_host RPC to support transferred host ownership
-- Description: Updates promote_to_host RPC to authorize ANY host (plans.host_id OR role = 'HOST')
--              and removes the arbitrary 2-additional-hosts limit.
...
BEGIN
  ...
  IF v_caller_id <> v_creator_id THEN
    ...
    IF v_caller_role IS NULL OR v_caller_role NOT IN ('HOST'::participant_role, 'CO_HOST'::participant_role) THEN
