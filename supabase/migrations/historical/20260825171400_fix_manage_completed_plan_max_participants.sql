-- Migration: Fix max_participants query in manage_completed_plan_participants RPC
-- Description: Fixes plan capacity selection from plans table while enforcing 24-hour participant management window.
...
BEGIN
  ...
  SELECT host_id, status, total_cost, scheduled_at, max_participants
  INTO v_host_id, v_plan_status, v_initial_total_cost, v_scheduled_at, v_orig_max_participants
  FROM public.plans
  WHERE id = p_plan_id
  FOR UPDATE;
