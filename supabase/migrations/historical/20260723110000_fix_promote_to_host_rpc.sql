-- ============================================================================
-- Migration: Fix promote_to_host RPC to cleanly handle existing hosts and enforce correct JOINED state
-- Description: Ensures promote_to_host RPC silently succeeds if target is already a host
--              and strictly validates that target participant is in JOINED state before promoting.
-- ============================================================================
...
BEGIN
  ...
  -- 7. Enforce Maximum Hosts Limit: max 2 additional hosts (3 hosts total)
  SELECT COUNT(*)
  ...
  IF v_additional_host_count >= 2 THEN
    RAISE EXCEPTION 'Maximum number of hosts reached'
      USING ERRCODE = '40901';
  END IF;
