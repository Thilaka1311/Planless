-- Migration: Multi-host support in promote_to_host RPC
-- Description: Promotes a participant to HOST role in plan_participants without removing or demoting existing hosts, supporting multiple hosts per plan.

-- 1. Ensure 'host_promoted' enum value exists in plan_activity_type
ALTER TYPE public.plan_activity_type ADD VALUE IF NOT EXISTS 'host_promoted';

-- 2. Update promote_to_host RPC
...
BEGIN
  ...
  -- 2. Fetch the plan creator/host_id from plans
  SELECT host_id
    INTO v_creator_id
    FROM public.plans
   WHERE id = p_plan_id;
  ...
  -- 3. Authorization: Caller must be the plans.host_id OR have role = 'HOST' in plan_participants
  ...
  -- 7. Promote target to HOST role in plan_participants (preserves existing hosts and plans.host_id)
