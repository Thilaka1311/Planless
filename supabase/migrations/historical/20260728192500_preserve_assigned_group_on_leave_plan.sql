-- Migration: Preserve assigned_group on voluntary leave_plan
-- Description: Updates leave_plan RPC to preserve existing assigned_group when participant leaves voluntarily.
--              remove_participant RPC continues clearing assigned_group = NULL on host removal.
...
BEGIN
  ...
  -- Update participant row: keep assigned_group unchanged (do NOT set to NULL)
  ...
  IF v_total_cost IS NOT NULL AND v_total_cost > 0 THEN
    ...
    ELSE
      v_new_cost_per_participant := v_total_cost;
    END IF;
