-- Migration: Update complete_plan RPC to support KEEP_CURRENT_COST mode
-- Description: Replaces CHARGE_NEW_ONLY with KEEP_CURRENT_COST, ensuring the existing per-person share is preserved for all eligible participants and the total expense increases accordingly. Also enforces that WAITLISTED participants do not receive an expense unless the completion flow transitions them to JOINED.
