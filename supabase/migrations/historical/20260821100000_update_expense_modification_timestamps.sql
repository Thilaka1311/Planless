-- Migration: Update expense modification timestamp behavior
-- Description: Ensures that when a pending expense is modified, created_at and updated_at are set to NOW() so the new pending version appears under the current date.
...
BEGIN
  ...
  SELECT payer_id, message_id, title, expense_type, COALESCE(status, 'PENDING')
    INTO v_payer_id, v_message_id, v_old_title, v_expense_type, v_curr_status
  ...
  -- Rule: When a pending expense is modified, refresh created_at = NOW() and updated_at = NOW()
  UPDATE public.wallet_expenses
     SET title = p_title,
         total_amount = p_total_amount,
         plan_id = p_plan_id,
         created_at = CASE WHEN v_curr_status != 'SETTLED' THEN NOW() ELSE created_at END,
         updated_at = NOW()
   WHERE id = p_expense_id;
