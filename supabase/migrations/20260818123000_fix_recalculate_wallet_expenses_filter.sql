-- Migration: Refine recalculate_wallet_expenses filter to rsvp_status IN ('JOINED', 'WAITLISTED')
-- Description: Ensures SKIPPED and INVITED participants are strictly excluded from plan-fee obligations even if assigned_group column retains a non-null string value.
