-- Migration: Fix Replacement Wallet Obligation on Keep Payment Resolution & Recalculation
-- Description:
-- 1. Updates resolve_paid_plan_leave_request so that KEEP_PAYMENT resolution also registers the replacement user as INVITED when provided.
-- 2. Updates recalculate_wallet_expenses to include participants where rsvp_status = 'JOINED' OR assigned_group = 'GOING', ensuring replacement participants receive their plan-fee obligation when moving to Going.
...
BEGIN
  ...
  -- Helper block: If replacement user ID is provided for either resolution, register them as INVITED
  ...
  ELSIF p_resolution = 'KEEP_PAYMENT' THEN
    -- Update target participant to SKIPPED / LEFT without deleting wallet row
    UPDATE public.plan_participants
       SET rsvp_status        = 'SKIPPED'::rsvp_status,
...
-- 2. Update recalculate_wallet_expenses RPC to include participants in Going
...
BEGIN
  ...
  ELSE
    ...
    FROM public.plan_participants
    WHERE plan_id = p_plan_id 
      AND (rsvp_status = 'JOINED' OR assigned_group = 'GOING') 
  ...
  WHERE plan_id = p_plan_id AND (rsvp_status = 'JOINED' OR assigned_group = 'GOING');
  ...
  -- Remove participant rows for users no longer in Going, EXCEPT PRESERVE SETTLED RECORDS
  DELETE FROM public.wallet_expense_participants
  WHERE expense_id = v_expense_id
    AND status != 'SETTLED'
    AND user_id NOT IN (
  ...
  -- Upsert participant shares for all Going participants, PRESERVING existing 'SETTLED' status
  ...
  WHERE pp.plan_id = p_plan_id AND (pp.rsvp_status = 'JOINED' OR pp.assigned_group = 'GOING')
