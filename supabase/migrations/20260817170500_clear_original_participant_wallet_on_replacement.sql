-- Migration: Immediately clear original participant wallet obligation on replacement confirmation
-- Description: Updates public.resolve_paid_plan_leave_request to immediately delete target participant's wallet_expense_participants row when resolution = 'REPLACED'. The replacement candidate is INVITED with no wallet row until they join.

DROP TRIGGER IF EXISTS trg_handle_replacement_wallet_transfer_on_join ON public.plan_participants;
DROP FUNCTION IF EXISTS public.handle_replacement_wallet_transfer_on_join();
...
BEGIN
  ...
  IF p_resolution = 'REPLACED' THEN
    ...
    -- IMMEDIATELY CLEAR target participant's wallet obligation for plan expense (NO transfer to replacement)
    ...
    IF v_expense_id IS NOT NULL THEN
      DELETE FROM public.wallet_expense_participants
       WHERE expense_id = v_expense_id AND user_id = p_target_user_id;
    END IF;
