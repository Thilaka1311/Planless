-- Migration: Update complete_plan RPC to support new attendees added during completion
-- Description: Automatically inserts any new attendee (e.g., added via Attendance Search) into plan_participants if marked as ATTENDED.
...
BEGIN
  ...
  -- Check if any remaining user_id in input does not belong to the plan
  IF EXISTS (
    SELECT 1
    FROM jsonb_array_elements(p_attendance_input) AS arr(item)
