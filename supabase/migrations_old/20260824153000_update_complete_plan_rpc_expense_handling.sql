-- Migration: Update complete_plan RPC to support participant count update & expense allocation modes
-- Description: Updates plans.max_participants to match final attended count, and handles PLAN_EXPENSE allocation recalculations atomically.
  ...
  IF p_expense_mode IN ('SPLIT_ALL', 'CHARGE_NEW_ONLY') AND v_final_count > 0 THEN
    ...
    IF v_plan_expense IS NOT NULL THEN
      ...
      IF p_expense_mode = 'SPLIT_ALL' THEN
        -- Recalculate share across all final attendees
        FOR v_participant IN
        ...
        LOOP
          IF v_participant.final_attendance = 'ATTENDED'::attendance_status THEN
            ...
            SET amount_owed = EXCLUDED.amount_owed,
                updated_at = now();
          ELSE
            -- Non-attendees owe 0 for this plan expense
      ...
      ELSIF p_expense_mode = 'CHARGE_NEW_ONLY' THEN
        -- Charge only newly added attendees without modifying existing participant shares
        FOR v_participant IN
          SELECT user_id
          FROM public.plan_participants
          WHERE plan_id = p_plan_id AND final_attendance = 'ATTENDED'::attendance_status
