-- Migration: 20260828113000_fix_replacement_participant_state.sql
-- Description: Updates skip_reason enum to include SKIPPED. Updates resolve_paid_plan_leave_request and replace_participant RPCs to transition the leaving participant to SKIPPED (with skip_reason = SKIPPED) and the replacement participant to INVITED. Properly handles inserting external friends as replacements.

ALTER TYPE public.skip_reason ADD VALUE IF NOT EXISTS 'SKIPPED';
...
BEGIN
  ...
  IF p_replacement_user_id IS NOT NULL THEN
    ...
    IF NOT FOUND THEN
      -- Insert replacement user directly as INVITED
    ...
    ELSE
      IF v_filtering_mode = 'ASSIGNED'::participant_filtering_type AND v_replacement_assigned_group = 'WAITLIST'::assigned_group_enum THEN
        -- Assigned Waitlist Replacement Logic: promote to GOING and INVITED
        UPDATE public.plan_participants
           SET assigned_group = 'GOING'::assigned_group_enum,
        ...
        UPDATE public.plan_participants pp
         ...
         WHERE pp.plan_id = p_plan_id AND pp.user_id = n.user_id;
      ELSE
        -- Standard replacement logic (Automatic waitlist or external participant): promote to INVITED
        UPDATE public.plan_participants
...
BEGIN
  ...
  -- 6. Transition or Insert replacement participant to INVITED
  IF FOUND THEN
