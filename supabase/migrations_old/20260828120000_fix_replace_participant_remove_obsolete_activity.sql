-- Migration: 20260828120000_fix_replace_participant_remove_obsolete_activity.sql
-- Description: Remove obsolete 'participant_invited' activity enum insert from replace_participant and resolve_paid_plan_leave_request RPCs while preserving rsvp_status = 'INVITED' for replacement participants, adhering strictly to Phase 3 plan_activity_type enum definitions.

-- 1. UPDATE replace_participant RPC
...
BEGIN
  ...
  -- 4. Check if replacement participant is already in plan_participants AND SAVE FOUND RESULT
  ...
  -- 5. Transition target participant to rsvp_status = SKIPPED and skip_reason = REPLACED
  ...
  -- 6. Transition or Insert replacement participant based on explicit v_replacement_found variable
  IF v_replacement_found THEN
    ...
    ELSE
       -- Non-waitlist external friend (e.g. SKIPPED, LEFT, REMOVED) -> invite them anew (RSVP = INVITED)
       UPDATE public.plan_participants
          SET assigned_group = CASE
                                 WHEN v_target_was_going AND v_filtering_mode = 'ASSIGNED'::participant_filtering_type THEN 'GOING'::assigned_group_enum
                                 WHEN v_target_was_going THEN NULL
  ...
  ELSE
    -- Completely new friend -> insert into plan_participants with RSVP = INVITED
...
BEGIN
  ...
  IF p_resolution = 'REPLACED' THEN
    ...
    -- Update target participant to rsvp_status = SKIPPED and skip_reason = REPLACED
    UPDATE public.plan_participants
       SET rsvp_status        = 'SKIPPED'::rsvp_status,
           skip_reason        = 'REPLACED'::skip_reason,
           leave_requested    = FALSE,
  ...
  IF p_replacement_user_id IS NOT NULL THEN
    ...
    IF v_replacement_found AND (v_replacement_assigned_group = 'GOING'::assigned_group_enum OR (v_filtering_mode = 'AUTOMATIC'::participant_filtering_type AND v_replacement_rsvp = 'JOINED'::rsvp_status)) THEN
      RAISE EXCEPTION 'Replacement user is already a joined participant' USING ERRCODE = '40000';
    END IF;

    IF NOT v_replacement_found THEN
    ...
    ELSE
      ...
      ELSE
        -- Standard replacement logic for non-waitlist (e.g. SKIPPED, LEFT, REMOVED) -> invite them anew (RSVP = INVITED)
        UPDATE public.plan_participants
