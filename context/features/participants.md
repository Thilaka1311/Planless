...
* Host taps a participant row to open `AssignedParticipantActions`:
  ...
  * Can adjust capacity (`update_plan_capacity` RPC), which promotes the top waitlisted candidates according to their manual sequence.
...
* **Standalone Header (`ParticipantHeader`)**:
  * Pinned top bar with back chevron (`ArrowLeft`), centered plan title and date/time subtitle, and trailing action icons (settings gear, activity timeline log).
...
### Plan Capacity Card (`PlanSizeCard` & `EditCapacityBottomSheet`)
* **PlanSizeCard**:
...
* **EditCapacityBottomSheet**: Modal bottom sheet invoked during wizard creation, featuring number wheel or stepper controls bounded between 2 and total invited count.
...
* **Status Badges & Chips**:
  * Leave Requested: Orange pill badge (`bg-orange-500/15 border border-orange-500/30 text-orange-400 text-[10px] px-2 py-0.5 rounded-full`) indicating pending host review.
  * Rejoined: Amber pill badge (`bg-amber-500/15 border border-amber-500/30 text-amber-400 text-[10px] px-2 py-0.5 rounded-full`) indicating skipped attendee requested to return.
...
* Fixed floating action button in bottom-right corner (`fixed z-40 w-12 h-12 rounded-full bg-[#FF6B2C] text-white flex items-center justify-center shadow-lg shadow-black/50 border border-white/20 active:scale-95 transition-all`).
...
|---|---|---|---|
| `PlanParticipantManagementWrapper` | `src/features/plans/screens/PlansScreen/PlansPreview/PlanParticipantManagementWrapper.tsx` | Mode router container. Inspects `plan.participantFiltering` / `plan.waitlistMode` and routes cleanly to either `AssignedParticipantContainer` or `AutomaticParticipantContainer`. Decouples all business logic, mutations, and bottom sheets between the two modes. | Consumed by `PlansPreviewScreen` and `PlanChatScreen`. |
| `AssignedParticipantContainer` | `src/features/participants/assigned/AssignedParticipantContainer.tsx` | Complete domain container for Assigned mode. Encapsulates host-guided capacity adjustments, waitlist drag-and-drop, manual swaps, replacement sheets, and cost splitting. | Renders `AssignedParticipantScreen` with Assigned bottom sheets. |
| `AutomaticParticipantContainer` | `src/features/participants/automatic/AutomaticParticipantContainer.tsx` | Complete domain container for Automatic mode. Encapsulates FCFS queue ordering based on `joined_queue_at`, deterministic auto-promotion, and cost splitting. Free of Assigned-specific drag-and-drop or manual swap state. | Renders `AutomaticParticipantScreen` with Automatic bottom sheets. |
| `ParticipantManagementScreen` | `src/features/participants/screens/ParticipantManagementScreen.tsx` | Wizard / presentation mode router. Inspects `props.waitlistMode` and delegates rendering to either `AssignedParticipantScreen` or `AutomaticParticipantScreen`. | Consumed during plan creation (`WhoIsActuallyComing`). |
...
| `EditCapacityBottomSheet` | `src/features/create/components/PlanSizeBottomsheet.tsx` | Standalone bottom sheet for adjusting plan capacity. Allows free stepper adjustments while open; commits changes automatically upon closing (backdrop tap, drag-to-dismiss, or handle tap) without a separate confirmation button. | Triggered by header capacity chips in `AutomaticParticipantScreen`, `AssignedParticipantScreen`, and `PlansPreviewScreen`. |
...
```text
[Host Opens Participants in Active Plan]
                 │
                 ▼
 [PlanParticipantManagementWrapper]
  ├── Mode: Assigned  ──► [AssignedParticipantContainer]
  │                        ├── Guided capacity movements (Move to Join / Waitlist)
  │                        ├── Drag-and-drop waitlist reordering
  │                        ├── Manual swaps & replacements
  │                        └── Renders AssignedParticipantScreen
  └── Mode: Automatic ──► [AutomaticParticipantContainer]
                           ├── FCFS queue partitioning via joined_queue_at
                           ├── Automatic capacity adjustment & auto-promotion
                           └── Renders AutomaticParticipantScreen
                 │
 ┌───────────────┴───────────────┐
 ▼                               ▼
[Automatic Mutation]            [Assigned Mutation]
  ...
  └── Re-renders active participant container with updated slots
```
...
* `update_plan_capacity(p_plan_id uuid, p_plan_size int, p_auto_promote boolean DEFAULT true)`: Updates `plan_size`. In both Automatic and Assigned modes, passing `p_auto_promote = false` updates `plan_size` without automated promotions or demotions. This powers the host-guided adjustment flows (`GuidedCapacityAdjustmentBottomSheet`), where:
  * **On capacity increase**: If waitlisted participants exist to fill newly available spots, the host is guided to select which waitlisted participants move to `Joined` / `Going`. Only the host-selected participants are moved, and the remaining waitlist and its exact ordering are preserved.
  * **On capacity decrease**: If the target capacity is lower than confirmed attendees, the host is guided to select which non-host attendees move to the waitlist.
  * **Maximum capacity cap**: In all modes (active plans and wizard), plan size is strictly capped by the total number of active invited participants in the plan (`limitToInvitedCount: true`). It can never exceed the total invited attendees.
