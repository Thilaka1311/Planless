...
import {
  JoinPlanAnimation,
  isJoinPlanAnimationCompleted,
} from "../components/JoinPlanAnimation";

interface JoinPlanOnboardingProps {
  onGetStarted: () => void;
  onBack?: () => void;
...
export function JoinPlanOnboarding({
  onGetStarted,
  onBack,
  onTouchStart,
  onTouchEnd,
  className = "",
}: JoinPlanOnboardingProps) {
  ...
  return (
    <div
      id="join_plan_onboarding_screen"
    ...
    >
      ...
      <div className="w-full shrink-0 pt-1 xs:pt-2 sm:pt-3 px-5 sm:px-8 md:px-10 flex flex-col items-center">
        <h1 className="text-[18px] xs:text-[20px] sm:text-[22px] md:text-[24px] font-sans font-bold tracking-tight text-white leading-[1.25] text-center max-w-[360px] xs:max-w-[400px] sm:max-w-[440px] mx-auto">
          <span className="block">Friends see the plan</span>
          <span className="block">and join with one tap</span>
        </h1>
      </div>

      {/* 2. [JOIN PLAN ANIMATION] Perfectly vertically centered between Heading bottom and Next Button top */}
      <div className="flex-1 w-full min-h-0 flex items-center justify-center px-4 py-2 sm:py-3 overflow-hidden">
        <JoinPlanAnimation onComplete={handleComplete} />
      </div>
        ...
        <div className="h-[46px] w-full flex items-center">
          <AnimatePresence>
            {showNext && (
              ...
              >
                <button
                  id="btn_onboarding_cta_join_plan"
                  type="button"
...
export default JoinPlanOnboarding;
