...
export const CreatePlanConfirmation: React.FC<CreatePlanConfirmationProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 gap-8 my-auto">
        ...
        <div className="relative flex items-center justify-center">
          ...
          <motion.div
            className="relative w-24 h-24 rounded-full overflow-hidden flex items-center justify-center border border-[#FF6B2C]/30 bg-[#FF6B2C]/10"
        ...
        {stage === "content" && (
          ...
          >
            ...
            <p className="text-[17px] font-bold text-zinc-400 tracking-tight">
              Plan Created!
            </p>
          </motion.div>
        )}
      ...
      {stage === "content" && (
        <motion.div
          className="px-5 pb-10 pt-4 flex flex-col items-center gap-3.5 w-full max-w-sm mx-auto"
          initial={prefersReducedMotion ? {} : { opacity: 0, y: 16 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }}
        >
          {/* Share — primary CTA: pill-shaped, orange background, white text */}
          <motion.button
            type="button"
            onClick={onCopyInviteLink}
            disabled={isCopying}
            className="w-full bg-[#FF6B2C] text-white py-4 rounded-full font-extrabold text-[12px] tracking-widest uppercase flex items-center justify-center gap-2 disabled:opacity-50 cursor-pointer select-none shadow-lg shadow-[#FF6B2C]/25"
          ...
          {onGoToPlans && (
            <motion.button
              type="button"
              onClick={onGoToPlans}
              className="w-full bg-transparent border-none text-zinc-400 hover:text-white py-2 font-bold text-[12px] tracking-widest uppercase flex items-center justify-center transition-colors cursor-pointer select-none mt-1"
              whileTap={prefersReducedMotion ? {} : { scale: 0.98 }}
              transition={springTransition}
            >
              Go to Plans
